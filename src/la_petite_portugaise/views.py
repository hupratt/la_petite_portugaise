...
class index(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        liste_events_en = Post.objects.all().filter(tag='event').order_by('timestamp').filter(timestamp__gte = datetime.datetime.now(tz=(settings.TIME_ZONE)))  # pylint: disable=no-member
        if language == 'en':
