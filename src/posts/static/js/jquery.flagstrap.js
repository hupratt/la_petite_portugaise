...
(function($) {
  ...
  $.flagStrap = function(element, options, i) {
    ...
    plugin.init = function() {
      ...
      if (
        plugin.settings.onSelect !== undefined &&
        plugin.settings.onSelect instanceof Function
      ) {
    ...
    var buildDropDownButton = function() {
      var selectedText = $(htmlSelect)
        .find("option")
        .first()
        .text();
      var selectedValue = $(htmlSelect)
        .find("option")
        .first()
        .val();
      ...
      if (selectedValue !== plugin.settings.placeholder.value) {
        ...
        var $selectedLabel = $("<i/>").addClass(
          "flagstrap-icon flagstrap-placeholder"
        );
      }
      ...
      var button = $("<button/>")
        ...
        .addClass(
          "btn " +
            plugin.settings.buttonType +
            " " +
            plugin.settings.buttonSize +
            " dropdown-toggle"
        )
        // .attr(
        //   "onClick",
        //   '$( ".dropdown-menu" ).attr("style", "z-index:500000;transform:translate3d(0px, 40px, 0px);")'
        // )
        .attr("style", "z-index:500000;")
        .html(buttonLabel);
    ...
    var buildDropDownButtonItemList = function() {
      var items = $("<ul/>")
        ...
        // .attr("style", { transform: "translate3d(0px, 40px, 0px);" })
      ...
      $(htmlSelect)
        ...
        .each(function() {
          ...
          if (value !== plugin.settings.placeholder.value) {
            var flagIcon = $("<i/>")
              .addClass("flagstrap-icon flagstrap-" + value.toLowerCase())
              .css("margin-right", plugin.settings.labelMargin);
          } else {
            var flagIcon = null;
          }
